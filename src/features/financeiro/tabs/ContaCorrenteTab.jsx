 ...
 * - ◄ CORRIGIDO: Definição interna de auxiliares (DocumentCard, Slot, IA) para garantir escopo e evitar crashes em produção [2].
...
export default function MotoristaForm({ onSubmit, initialData = {}, onCancel, isReadOnly = false, onCriarProprietario }) {
  ...
  // ◄ NOVO (SINTAXE CORRIGIDA): Componentes auxiliares movidos para o escopo interno do MotoristaForm [2]
  // Isto garante o fecho de escopo (closure) e evita erros de ReferenceError em produção [2].
  ...
  return (
    <form onSubmit={(e) => e.preventDefault()} className="space-y-1 max-h-[75vh] overflow-y-auto pr-3.5 custom-scrollbar">
      ...
      {/* DADOS VITAIS E DE IDENTIFICAÇÃO (Padding e margem otimizados) */}
